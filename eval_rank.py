...
def main(cfg: DictConfig) -> None:
    ...
    # test
    if cfg.model.test.get_from_file:
        ...
        for i in range(len(targets_tokens)):
            ...
            logs.append({
                ...
                'HITS@3': 1.0 if 1 <= rank <= 3 else 0.0,
                'HITS@10': 1.0 if 1 <= rank <= 10 else 0.0,
            })
            escape_flg = False
            rank = 3850 if cfg.data.name == "FB15kET" else 49979
        res = dict()
        ...
        for metric in res:
            logging.info('%s: %f' % (metric, res[metric]))
